<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet version="2.0"
	xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
	xmlns:fo="http://www.w3.org/1999/XSL/Format">
	<xsl:output method="xml" indent="yes" />

	<xsl:template match="/">
		<fo:root xmlns:fo="http://www.w3.org/1999/XSL/Format"
			font-family="Arial, Helvetica, sans-serif">

			<fo:layout-master-set>
				<fo:simple-page-master master-name="A4" page-width="210mm" page-height="297mm" margin-top="1cm" margin-bottom="1cm" margin-left="2cm" margin-right="2cm">
					<fo:region-body margin-top="3cm" />
					<fo:region-before precedence="true" extent="3cm" />
				</fo:simple-page-master>
			</fo:layout-master-set>

			<fo:page-sequence master-reference="A4" border-style="solid">
				<fo:static-content flow-name="xsl-region-before">
				
					<fo:block border-style="solid" text-align="center" padding="2mm">
							<fo:inline>
								<fo:external-graphic
									src="url('data:image/png;base64,')" />
							</fo:inline>
						</fo:block>
				</fo:static-content> 
				<fo:flow flow-name="xsl-region-body">		
				
						<fo:block font-size="10pt" text-align="center"
							space-after="15mm" >
							<fo:block font-weight="bold" space-before="10mm" space-after="5mm">ATTO DI ADESIONE UNICO</fo:block>
							<fo:block space-after="1mm">PERCORSI DI ISTRUZIONE E FORMAZIONE PROFESSIONALE (IeFP) II ANNO – A.F. 2015/2016</fo:block>
							<fo:block space-after="1mm">di cui al Decreto della Struttura Istruzione e formazione professionale,</fo:block>
							<fo:block space-after="1mm">Tecnica superiore e Diritto allo studio del 31/07/2015, n. 6557</fo:block>
						</fo:block>
						<fo:block font-size="10pt" text-align="justify" space-after="5mm">
							<fo:inline>Il/la sottoscritto/a</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/AttoUnico_FirmatarioNome" />
							</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/AttoUnico_FirmatarioCognome" />
							</fo:inline>
							<xsl:choose>
								<xsl:when test="/_/AttoUnico_FirmatarioNascitaProvincia[text()='EEE']">
									<fo:inline>, nato/a a</fo:inline>
									<fo:inline padding-left="1mm">
										<xsl:value-of select="/_/AttoUnico_FirmatarioNascitaComuneDn" />
									</fo:inline>
									<fo:inline padding-left="1mm">il</fo:inline>
								</xsl:when>
								<xsl:otherwise>
									<fo:inline>, nato/a a</fo:inline>
									<fo:inline padding-left="1mm">
										<xsl:value-of select="/_/AttoUnico_FirmatarioNascitaComuneDn" />
									</fo:inline>
									<fo:inline>, prov. </fo:inline>
									<fo:inline>
										<xsl:value-of select="/_/AttoUnico_FirmatarioNascitaProvinciaDn" />
									</fo:inline>
									<fo:inline>, il</fo:inline>
								</xsl:otherwise>
							</xsl:choose>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/AttoUnico_FirmatarioNascitaData" />
							</fo:inline>
							<fo:inline>, Cod. Fiscale</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/AttoUnico_FirmatarioCodiceFiscale" />
							</fo:inline>
							<fo:inline>, in qualità di</fo:inline>
							<xsl:choose>
								<xsl:when test="/_/AttoUnico_FirmatarioRappresentanteLegale[text()='true']">
									<fo:inline padding-left="1mm">rappresentante legale</fo:inline>
								</xsl:when>
								<xsl:otherwise>
									<fo:inline padding-left="1mm">altro soggetto con potere di firma</fo:inline>
								</xsl:otherwise>
							</xsl:choose>
							<fo:inline>, dell’Operatore accreditato/autorizzato</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/Richiedente_Denominazione" />
							</fo:inline>
							<fo:inline padding-left="1mm">con sede in</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/Richiedente_SedeLegaleComuneDn" />
							</fo:inline>
							<fo:inline padding-left="1mm">via/piazza</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/Richiedente_SedeLegaleIndirizzo" />
							</fo:inline>
							<fo:inline>, CAP</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/Richiedente_SedeLegaleCap" />
							</fo:inline>
							<fo:inline padding-left="1mm">Prov.</fo:inline>
							<fo:inline padding-left="1mm">
								<xsl:value-of select="/_/Richiedente_SedeLegaleProvinciaDn" />
							</fo:inline>
						</fo:block>
						<fo:block font-weight="bold" font-size="10pt" text-align="center"
							space-after="2mm">PREMESSO CHE</fo:block>
						<fo:block font-size="10pt" text-align="justify" space-after="5mm">l’ammissione al finanziamento relativo all’iniziativa comporta da parte dell’Operatore l’esecuzione delle doti nel rispetto delle prescrizioni contenute nel provvedimento attuativo, delle normative comunitarie, nazionali e regionali;</fo:block>
						<fo:block font-weight="bold" font-size="10pt" text-align="center"
							space-after="2mm">accetta di</fo:block>
						<fo:table table-layout="fixed" width="170mm" space-after="2mm"
							border-spacing="0pt 2pt">
							<fo:table-column column-width="3mm" />
							<fo:table-column column-width="167mm" />
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell font-size="10pt" text-align="right" padding-before="1mm"
										padding-after="1mm">
										<fo:block>1.</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" padding-left="2mm" text-align="justify"
										padding-after="1mm">
										<fo:block>realizzare i servizi indicati nei PIP di propria competenza, entro i termini stabiliti, nel rispetto delle normative comunitarie, nazionali e regionali;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" text-align="right" padding-before="1mm"
										padding-after="1mm">
										<fo:block>2.</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" padding-left="2mm" text-align="justify"
										padding-after="1mm">
										<fo:block>rispettare la normativa in materia fiscale, previdenziale e di sicurezza dei lavoratori e dei partecipanti impegnati nella iniziativa approvata;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" text-align="right" padding-before="1mm"
										padding-after="1mm">
										<fo:block>3.</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" padding-left="2mm" text-align="justify"
										padding-after="1mm">
										<fo:block>consentire le attività di verifica e controllo sulla realizzazione delle doti da parte dei funzionari regionali, nazionali e comunitari competenti;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" text-align="right" padding-before="1mm"
										padding-after="1mm">
										<fo:block>4.</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" padding-left="2mm" text-align="justify"
										padding-after="1mm">
										<fo:block>fornire informazioni sui destinatari finali su richiesta di Regione per fini statistici e di monitoraggio;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" text-align="right" padding-before="1mm"
										padding-after="1mm">
										<fo:block>5.</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" padding-left="2mm" text-align="justify"
										padding-after="1mm">
										<fo:block>adempiere agli obblighi definiti dal presente Atto di Adesione per tutta la durata dell’iniziativa.</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
						</fo:table>
						<fo:block font-weight="bold" font-size="10pt" text-align="justify"
							space-after="5mm">Il sottoscritto, consapevole, in caso di dichiarazioni mendaci, della responsabilità penale ex art. 76 del DPR 445/2000, nonché della decadenza dal contributo concesso ex art. 75 DPR 445/2000</fo:block>
						<fo:block font-weight="bold" font-size="10pt" text-align="center"
							space-after="2mm">Dichiara</fo:block>
						<fo:table table-layout="fixed" width="170mm" space-after="5mm"
							border-spacing="0pt 2pt">
							<fo:table-column column-width="3mm" />
							<fo:table-column column-width="167mm" />
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>che non sono intervenute variazioni rispetto ai requisiti di accreditamento e che eventuali successive variazioni verranno comunicate tempestivamente alla Regione Lombardia - Direzione Generale Istruzione, Formazione e Lavoro;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di non percepire altri finanziamenti per l’erogazione dei servizi e di impegnarsi a comunicare immediatamente a Regione Lombardia la presenza di altri finanziamenti di organismi pubblici per i servizi rivolti ai destinatari;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di non essere sottoposto a misure, giudiziarie o amministrative, che limitano la possibilità giuridica di contrattare con la P.A.;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di non essere sottoposto a misure di prevenzione o ad altri impedimenti previsti dalla legislazione antimafia;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di accettare le condizioni economiche previste da Regione Lombardia e di vincolarsi a non richiedere erogazioni di somme a  titolo obbligatorio ai destinatari per i servizi previsti nei PIP; </fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di essere consapevole della facoltà di Regione Lombardia di recuperare somme indebitamente erogate qualora a seguito delle attività di verifica e controllo si rilevassero irregolarità nella realizzazione dei PIP;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di impegnarsi ad accettare le eventuali modifiche all’assetto regolamentare che si rendessero necessarie per effetto dell’entrata in vigore di nuove disposizioni comunitarie, nazionali o regionali;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>di essere consapevole che l’adesione alle regole del Manuale Operatore di cui al d.d.u.o. n. 1319 del 22 febbraio 2012 e successive modifiche e integrazioni è subordinata alla comunicazione di accettazione della Dichiarazione riassuntiva unica da parte di Regione Lombardia.</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
						</fo:table>
			
					<fo:block border-style="solid" padding="2mm">
						<fo:block font-weight="bold" font-size="10pt" text-align="center"
							space-after="2mm">Si impegna altresì</fo:block>
						<fo:table table-layout="fixed" width="170mm" space-after="5mm"
							border-spacing="0pt 2pt">
							<fo:table-column column-width="3mm" />
							<fo:table-column column-width="167mm" />
							<fo:table-body>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>a fornire ai destinatari presi in carico informazioni sulle modalità di finanziamento;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>a fornire ai Destinatari che saranno indicati nei PIP tutte le informazioni necessarie sia sulla realizzazione e sugli obblighi del piano da seguire, sia sugli aspetti amministrativi/contabili della gestione della dote e sulle conseguenze derivanti dalla mancata frequenza del percorso indicato;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>a concordare con i Destinatari il percorso di fruizione dei servizi come riportati nei PIP;</fo:block>
									</fo:table-cell>
								</fo:table-row>
								<fo:table-row>
									<fo:table-cell font-size="10pt" padding-before="1mm"
										padding-after="1mm">
										<fo:block>•</fo:block>
									</fo:table-cell>
									<fo:table-cell font-size="10pt" padding-before="1mm" text-align="justify"
										padding-after="1mm">
										<fo:block>a conservare la documentazione necessaria a comprovare l’erogazione dei servizi relativi alle doti per cui ha preso in carico il destinatario, nel rispetto delle disposizione del Regolamento (CE) 1303/2013.</fo:block>
									</fo:table-cell>
								</fo:table-row>
							</fo:table-body>
						</fo:table>
						<fo:block font-size="10pt" text-align="left" space-before="5mm">
							<fo:inline><xsl:value-of select="/_/Richiedente_SedeLegaleComuneDn" /></fo:inline>
							<fo:inline>, lì</fo:inline>
							<fo:inline padding-left="1mm"><xsl:value-of select="/_/Domanda_Data" /></fo:inline>
						</fo:block>
						<fo:block font-size="10pt" text-align="left" space-before="5mm">
							<fo:inline>L'operatore</fo:inline>
							<fo:inline padding-left="1mm"><xsl:value-of select="/_/AttoUnico_FirmatarioNome" /></fo:inline>
							<fo:inline padding-left="1mm"><xsl:value-of select="/_/AttoUnico_FirmatarioCognome" /></fo:inline>
						</fo:block>
						<fo:block font-size="7pt" text-align="left" margin-left="19mm">
							<fo:inline>Firma CRS</fo:inline>
						</fo:block>
					</fo:block>
		
					</fo:flow>
				</fo:page-sequence>
			</fo:root>
		</xsl:template>

</xsl:stylesheet>